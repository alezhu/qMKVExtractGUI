...
namespace qmkv::extract
{
  ...
  QString processPairs(
      ...
      std::function<QString(
          QString, const std::pair<QLatin1StringView, QLatin1StringView> &)>
          fn)
  {
    auto result = str;
  ...
  {
    auto result = processPairs(argString, [](auto str, auto pair)
                               { return str.replace(pair.first, pair.second); });
    return result;
  }
  ...
  {
    auto result = processPairs(argString, [](auto str, auto pair)
                               { return str.replace(pair.second, pair.first); });
    return result;
  }
